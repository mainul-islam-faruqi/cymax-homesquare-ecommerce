...
export function resolveLinks(
  ...
  if (item !== null && typeof item === 'object') {
    if (item?.sys?.type ?? false) {
      ...
      if (item?.sys?.linkType === 'Asset') {
        ...
        if ((includes?.Asset?.length ?? 0) > 0) {
          return copyWith(
            includes.Asset.find((x: any) => x.sys.id === item.sys.id)
          )
        ...
        if ((includes?.Entry?.length ?? 0) > 0) {
          const inObject = includes.Entry.find(
            (x: contentfulEntry) => x.sys.id === item.sys.id
          )
    ...
    Object.entries(item).forEach(([key, value]: [string, any]) => {
      ...
      let result = resolveLinks(
        value,
        includes,
        key === 'fields' ? level + 1 : level,
        maxLevel,
        copyWith,
        key,
      ...
      if (result !== undefined) {
        ...
        value?.linkToEntry &&
        parent &&
        ['cta', 'target'].includes(parent)
      ) {
        value?.linkToEntry?.fields?.items &&
          delete value.linkToEntry.fields.items
